...
status_t FXOS_Init(fxos_handle_t *fxos_handle, fxos_config_t *config) {
    ...
    if (FXOS_WriteReg(fxos_handle, CTRL_REG1, tmp[0] & (uint8_t)~ACTIVE_MASK) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    ...
    /* Disable the FIFO */
    if (FXOS_WriteReg(fxos_handle, F_SETUP_REG, F_MODE_DISABLED) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    ...
    if (FXOS_WriteReg(fxos_handle, CTRL_REG2,
                      SLPE_MASK | SMOD_LOW_POWER | MOD_HIGH_RES) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
#else
    /* enable auto-sleep, low power in sleep, high res in wake */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG2, MOD_HIGH_RES) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    ...
    if (FXOS_WriteReg(fxos_handle, M_CTRL_REG1,
                      (M_RST_MASK | M_OSR_MASK | M_HMS_MASK)) !=
        kStatus_Success) {
        return kStatus_Fail;
    }

    /* Enable hyrid mode auto increment using M_CTRL_REG2 */
    if (FXOS_WriteReg(fxos_handle, M_CTRL_REG2, (M_HYB_AUTOINC_MASK)) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    ...
    if (FXOS_WriteReg(fxos_handle, FF_MT_CFG_REG,
                      XEFE_MASK | YEFE_MASK | ELE_MASK | OAE_MASK) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
#endif
    ...
    if (FXOS_WriteReg(fxos_handle, XYZ_DATA_CFG_REG, FULL_SCALE_4G) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
#ifdef EN_INTERRUPTS
    /* enable data-ready, auto-sleep and motion detection interrupts */
    /* FXOS1_WriteRegister(CTRL_REG4, INT_EN_DRDY_MASK | INT_EN_ASLP_MASK |
     * INT_EN_FF_MT_MASK); */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG4, 0x0) != kStatus_Success) {
        return kStatus_Fail;
    }
    /* route data-ready interrupts to INT1, others INT2 (default) */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG5, INT_CFG_DRDY_MASK) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    /* enable ffmt as a wake-up source */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG3, WAKE_FF_MT_MASK) !=
        kStatus_Success) {
        return kStatus_Fail;
    }
    /* finally activate accel_device with ASLP ODR=0.8Hz, ODR=100Hz, FSR=2g */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG1,
                      HYB_ASLP_RATE_0_8HZ | HYB_DATA_RATE_100HZ |
                          ACTIVE_MASK) != kStatus_Success) {
        return kStatus_Fail;
    }
#else
    /* Setup the ODR for 50 Hz and activate the accelerometer */
    if (FXOS_WriteReg(fxos_handle, CTRL_REG1,
                      (HYB_DATA_RATE_200HZ | ACTIVE_MASK)) != kStatus_Success) {
        return kStatus_Fail;
    }
#endif
...
status_t FXOS_ReadSensorData(fxos_handle_t *fxos_handle,
                             fxos_data_t *sensorData) {
    status_t status = kStatus_Success;
    uint8_t tmp_buff[6] = {0};
    uint8_t i = 0;

    if ((FXOS_ReadReg(fxos_handle, OUT_X_MSB_REG, tmp_buff, 6)) !=
        kStatus_Success) {
        status = kStatus_Fail;
    }
    ...
    if ((FXOS_ReadReg(fxos_handle, M_OUT_X_MSB_REG, tmp_buff, 6)) !=
        kStatus_Success) {
        status = kStatus_Fail;
    }
...
status_t FXOS_ReadReg(fxos_handle_t *handle, uint8_t reg, uint8_t *val,
    ...
    return handle->I2C_ReceiveFunc(handle->slaveAddress, reg, 1, val,
                                   bytesNumber);
}
