...
Shared::Region LineTracer::getDistances(const Shared::Image &image, bool hasPrevDistance, bool forceSearchRegions) {
...
#define outOfBounds(index)  (index <= Shared::Image::BLACK_COUNT) || (index >= CAMERA_LINE_LENGTH - Shared::Image::BLACK_COUNT - 1)
	...
	while (std::abs(i) < PREV_LINE_SEARCH_REGION) {
		...
		if (!isFoundRight()) {
			...
			if (rightRightColor == Shared::Image::COLOR_BLACK && leftRightColor == Shared::Image::COLOR_WHITE) {
				setFoundRight(true);
			}
		}
...
uint32_t LineTracer::getListSize() {
