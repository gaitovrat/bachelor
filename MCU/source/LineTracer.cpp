...
LineTracer::LineTracer(const uint32_t listSize)
    : m_listSize{listSize}, m_blackRegionsCount{0}, m_whiteRegionsCount{0},
      m_hasLeft{false}, m_hasRight{false} {}
...
void LineTracer::Reset() { this->m_imageRegionList.clear(); }
...
std::pair<uint8_t, uint8_t> &LineTracer::DistancesPair() {
    ...
    m_unchangedLeft =
        ((m_imageRegionList.back().Left >=
         ...
         (m_imageRegionList.back().Left <= m_currentAverage.first + TOLERANCE));
...
size_t LineTracer::ListSize() { return m_imageRegionList.size(); }

bool LineTracer::HasLeft() { return m_hasLeft; }

bool LineTracer::HasRight() { return m_hasRight; }

Shared::Region LineTracer::LastRawRegion() { return m_imageRegionList.back(); }

bool LineTracer::UnchangedLeft() const { return m_unchangedLeft; }
bool LineTracer::UnchangedRight() const { return m_unchangedRight; }
