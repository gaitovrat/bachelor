 ...
 * that user wishes to use.
 *
 * @param base       FTM peripheral base address
 * @param syncMethod Synchronization methods to use to update buffered
 * registers. This is a logical OR of members of the enumeration
 ...
 * use as loading points for FTM register update. When using this it is not
 * required to use PWM synchnronization.
 *
 * @param base         FTM peripheral base address
...
static void FTM_SetPwmSync(FTM_Type *base, uint32_t syncMethod) {
    ...
    reg = base->COMBINE;
    for (chnlNumber = 0;
         chnlNumber < ((uint8_t)FSL_FEATURE_FTM_CHANNEL_COUNTn(base) / 2U);
         chnlNumber++) {
        /* Enable PWM synchronization of registers C(n)V and C(n+1)V */
        reg |= (1UL << (FTM_COMBINE_SYNCEN0_SHIFT +
                        (FTM_COMBINE_COMBINE1_SHIFT * chnlNumber)));
    }
    base->COMBINE = reg;

    reg = base->SYNCONF;

    /* Use enhanced PWM synchronization method. Use PWM sync to update register
     * values */
    ...
    if ((syncMethod & FTM_SYNC_SWSYNC_MASK) != 0U) {
        /* Enable needed bits for software trigger to update registers with its
         * buffer value */
        reg |= (FTM_SYNCONF_SWRSTCNT_MASK | FTM_SYNCONF_SWWRBUF_MASK |
                FTM_SYNCONF_SWINVC_MASK | FTM_SYNCONF_SWSOC_MASK |
                FTM_SYNCONF_SWOM_MASK);
    }

    if ((syncMethod & (FTM_SYNC_TRIG0_MASK | FTM_SYNC_TRIG1_MASK |
                       FTM_SYNC_TRIG2_MASK)) != 0U) {
        /* Enable needed bits for hardware trigger to update registers with its
         * buffer value */
        reg |= (FTM_SYNCONF_HWRSTCNT_MASK | FTM_SYNCONF_HWWRBUF_MASK |
                FTM_SYNCONF_HWINVC_MASK | FTM_SYNCONF_HWSOC_MASK |
...
static void FTM_SetReloadPoints(FTM_Type *base, uint32_t reloadPoints) {
    ...
    reg = base->COMBINE;
    for (chnlNumber = 0; chnlNumber < ((uint32_t)chnlCount / 2U);
         chnlNumber++) {
...
status_t FTM_Init(FTM_Type *base, const ftm_config_t *config) {
    ...
    /* This function does not support the current instance, please use the TPM
     * driver. */
    ...
    if ((config->pwmSyncMode & (uint32_t)((uint32_t)FTM_SYNC_TRIG0_MASK |
                                          (uint32_t)FTM_SYNC_TRIG1_MASK |
                                          (uint32_t)FTM_SYNC_TRIG2_MASK |
                                          (uint32_t)FTM_SYNC_SWSYNC_MASK)) ==
        0U) {
    ...
    if (0 != FSL_FEATURE_FTM_IS_BASIC_FEATURE_ONLY_INSTANCEn(base)) {
        ...
        /* Configure the fault mode, enable FTM mode and disable write
         * protection */
    ...
    base->DEADTIME = (0u |
#if defined(FSL_FEATURE_FTM_HAS_EXTENDED_DEADTIME_VALUE) &&                    \
    (FSL_FEATURE_FTM_HAS_EXTENDED_DEADTIME_VALUE)
 ...
 * param numOfChnls  Number of channels to configure; This should be the size of
 * the array passed in param mode        PWM operation mode, options available
 * in enumeration ::ftm_pwm_mode_t param pwmFreq_Hz  PWM signal frequency in Hz
...
status_t FTM_SetupPwm(FTM_Type *base,
    ...
    /* Return an error in case we overflow the registers, probably would require
     * changing clock source to get the desired frequency */
    if (mod > 65535U) {
    ...
    for (i = 0; i < numOfChnls; i++) {
         ...
         */
        if (((uint8_t)chnlParams->chnlNumber >=
             (uint8_t)FSL_FEATURE_FTM_CHANNEL_COUNTn(base)) ||
    ...
    (FSL_FEATURE_FTM_HAS_ENABLE_PWM_OUTPUT)
            ...
            if (((uint32_t)chnlParams->chnlNumber) >=
                ((uint32_t)FSL_FEATURE_FTM_CHANNEL_COUNTn(base) / 2U)) {
                return kStatus_InvalidArgument;
            }
            ...
            if (mode == kFTM_EdgeAlignedCombinedPwm) {
                ...
                /* Return error if requested value is greater than the max
                 * allowed */
                if ((chnlParams->firstEdgeDelayPercent +
                     chnlParams->dutyCyclePercent) > 100U) {
                    return kStatus_OutOfRange;
                }
            ...
            reg = base->CONTROLS[(((uint32_t)chnlParams->chnlNumber) * 2U) + 1U]
                      .CnSC;
            reg &= ~(FTM_CnSC_MSA_MASK | FTM_CnSC_MSB_MASK |
                     FTM_CnSC_ELSA_MASK | FTM_CnSC_ELSB_MASK);
            ...
            base->CONTROLS[(((uint32_t)chnlParams->chnlNumber) * 2U) + 1U]
                .CnSC = reg;

            /* Set the combine bit for the channel pair */
            base->COMBINE |= (1UL << (FTM_COMBINE_COMBINE0_SHIFT +
                                      (FTM_COMBINE_COMBINE1_SHIFT *
                                       (uint32_t)chnlParams->chnlNumber)));

            /* Set the channel pair values */
            base->CONTROLS[((uint32_t)chnlParams->chnlNumber) * 2U].CnV =
                cnvFirstEdge;
            base->CONTROLS[(((uint32_t)chnlParams->chnlNumber) * 2U) + 1U].CnV =
                cnvFirstEdge + cnv;
    ...
    (FSL_FEATURE_FTM_HAS_ENABLE_PWM_OUTPUT)
            ...
            FTM_SetPwmOutputEnable(
                base,
                (ftm_chnl_t)(uint8_t)((uint8_t)chnlParams->chnlNumber * 2U),
                true);
            FTM_SetPwmOutputEnable(
                base,
                (ftm_chnl_t)(uint8_t)((uint8_t)chnlParams->chnlNumber * 2U +
            ...
            FTM_SetComplementaryEnable(base, chnlParams->chnlNumber,
                                       chnlParams->enableComplementary);
            /* Enable/Disable Deadtime insertion on the channel pair */
            FTM_SetDeadTimeEnable(base, chnlParams->chnlNumber,
                                  chnlParams->enableDeadtime);
        }
        chnlParams++;
    }
 ...
 * this represents the channel pair number param currentPwmMode    The current
 * PWM mode set during PWM setup param dutyCyclePercent  New PWM pulse width;
 * The value should be between 0 to 100 0=inactive signal(0% duty cycle)...
...
status_t FTM_UpdatePwmDutycycle(FTM_Type *base, ftm_chnl_t chnlNumber,
    ...
    if ((currentPwmMode == kFTM_EdgeAlignedPwm) ||
        ...
        /* This check is added for combined mode as the channel number should be
         * the pair number */
        if ((uint32_t)chnlNumber >=
            ((uint32_t)FSL_FEATURE_FTM_CHANNEL_COUNTn(base) / 2U)) {
            return kStatus_InvalidArgument;
        }
        ...
        if (currentPwmMode == kFTM_CenterAlignedCombinedPwm) {
            ...
            if (((cnvFirstEdge != 0U) && (cnv == (mod + 1U))) ||
                (((cnvFirstEdge + cnv) > mod) && (cnv < mod))) {
                /* Return error if firstEdgeDelayPercent + dutyCyclePercent >
                 * 100 */
                return kStatus_OutOfRange;
            }
        }
        base->CONTROLS[((uint32_t)chnlNumber * 2U)].CnV = cnvFirstEdge;
        base->CONTROLS[((uint32_t)chnlNumber * 2U) + 1U].CnV =
            cnvFirstEdge + cnv;
    }
    return kStatus_Success;
}
...
void FTM_UpdateChnlEdgeLevelSelect(FTM_Type *base, ftm_chnl_t chnlNumber,
    ...
    reg |= ((uint32_t)level << FTM_CnSC_ELSA_SHIFT) &
           (FTM_CnSC_ELSA_MASK | FTM_CnSC_ELSB_MASK);
...
status_t FTM_SetupPwmMode(FTM_Type *base,
    ...
    (FSL_FEATURE_FTM_HAS_ENABLE_PWM_OUTPUT)
            ...
            base->CONTROLS[((uint32_t)chnlParams->chnlNumber) * 2U].CnV =
                cnvFirstEdge;
            base->CONTROLS[(((uint32_t)chnlParams->chnlNumber) * 2U) + 1U].CnV =
                cnvFirstEdge + chnlParams->dutyValue;
...
void FTM_SetupInputCapture(FTM_Type *base, ftm_chnl_t chnlNumber,
    ...
    base->COMBINE &=
        ~(1UL << (FTM_COMBINE_COMBINE0_SHIFT +
                  (FTM_COMBINE_COMBINE1_SHIFT * ((uint32_t)chnlNumber >> 1))));
    /* Clear the dual edge capture mode because it's it's higher priority */
    base->COMBINE &=
        ~(1UL << (FTM_COMBINE_DECAPEN0_SHIFT +
                  (FTM_COMBINE_COMBINE1_SHIFT * ((uint32_t)chnlNumber >> 1))));
    ...
    if (chnlNumber < kFTM_Chnl_4) {
        reg = base->FILTER;
        reg &= ~((uint32_t)FTM_FILTER_CH0FVAL_MASK
                 << (FTM_FILTER_CH1FVAL_SHIFT * (uint32_t)chnlNumber));
        reg |=
            (filterValue << (FTM_FILTER_CH1FVAL_SHIFT * (uint32_t)chnlNumber));
        base->FILTER = reg;
    }
#if defined(FSL_FEATURE_FTM_HAS_ENABLE_PWM_OUTPUT) &&                          \
    (FSL_FEATURE_FTM_HAS_ENABLE_PWM_OUTPUT)
 ...
 * condition is met param compareValue Value to be programmed in the CnV
 * register.
 */
...
void FTM_SetupDualEdgeCapture(FTM_Type *base, ftm_chnl_t chnlPairNumber,
    ...
    if (chnlPairNumber < kFTM_Chnl_4) {
        reg = base->FILTER;
        reg &= ~((uint32_t)FTM_FILTER_CH0FVAL_MASK
                 << (FTM_FILTER_CH1FVAL_SHIFT * (uint32_t)chnlPairNumber));
        reg |= (filterValue
                << (FTM_FILTER_CH1FVAL_SHIFT * (uint32_t)chnlPairNumber));
        base->FILTER = reg;
    }
...
void FTM_SetupFaultInput(FTM_Type *base, ftm_fault_input_t faultNumber,
    ...
    if (faultParams->useFaultFilter) {
        ...
        base->FLTCTRL |=
            ((uint32_t)FTM_FLTCTRL_FFLTR0EN_MASK
             << (FTM_FLTCTRL_FFLTR0EN_SHIFT + (uint32_t)faultNumber));
    } else {
        /* Disable the fault filter */
        base->FLTCTRL &=
            ~((uint32_t)FTM_FLTCTRL_FFLTR0EN_MASK
    ...
    if (faultParams->enableFaultInput) {
        ...
        base->FLTCTRL |=
            ((uint32_t)FTM_FLTCTRL_FAULT0EN_MASK << (uint32_t)faultNumber);
    } else {
