...
void ADC16_Init(ADC_Type *base, const adc16_config_t *config) {
    ...
    /* ADCx_CFG2. */
    tmp32 = base->CFG2 & ~(ADC_CFG2_ADACKEN_MASK | ADC_CFG2_ADHSC_MASK |
 ...
 * available settings. The default values are as follows. code
...
status_t ADC16_DoAutoCalibration(ADC_Type *base) {
     ...
     * Remember the hardware trigger state here and restore it later if the
    ...
    while (0U == ((uint32_t)kADC16_ChannelConversionDoneFlag &
                  ADC16_GetChannelStatusFlags(base, 0U))) {
        /* Check the CALF when the calibration is active. */
        if (0U != ((uint32_t)kADC16_CalibrationFailedFlag &
                   ADC16_GetStatusFlags(base))) {
            status = kStatus_Fail;
            break;
        }
    }
    ...
    if (0U !=
        ((uint32_t)kADC16_CalibrationFailedFlag & ADC16_GetStatusFlags(base))) {
        status = kStatus_Fail;
    }
    if (kStatus_Success !=
        status) /* Check if the calibration process is succeed. */
    {
        return status;
    }
...
void ADC16_SetHardwareCompareConfig(
    ...
    uint32_t tmp32 = base->SC2 & ~(ADC_SC2_ACFE_MASK | ADC_SC2_ACFGT_MASK |
                                   ADC_SC2_ACREN_MASK);
 ...
 * param config        Pointer to the "adc16_channel_config_t" structure for the
 * conversion channel.
 */
void ADC16_SetChannelConfig(ADC_Type *base, uint32_t channelGroup,
    ...
    uint32_t sc1 =
        ADC_SC1_ADCH(config->channelNumber); /* Set the channel number. */
