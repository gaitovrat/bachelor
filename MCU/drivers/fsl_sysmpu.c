...
void SYSMPU_Init(SYSMPU_Type *base, const sysmpu_config_t *config) {
    ...
    for (count = 1; count < (uint32_t)FSL_FEATURE_SYSMPU_DESCRIPTOR_COUNT;
         count++) {
...
void SYSMPU_GetHardwareInfo(SYSMPU_Type *base,
    ...
    uint32_t regionsNumber_temp =
        (cesReg & SYSMPU_CESR_NRGD_MASK) >> SYSMPU_CESR_NRGD_SHIFT;

    hardwareInform->hardwareRevisionLevel =
        (uint8_t)((cesReg & SYSMPU_CESR_HRL_MASK) >> SYSMPU_CESR_HRL_SHIFT);
    hardwareInform->slavePortsNumbers =
        (uint8_t)((cesReg & SYSMPU_CESR_NSP_MASK) >> SYSMPU_CESR_NSP_SHIFT);
    hardwareInform->regionsNumbers =
        (sysmpu_region_total_num_t)(regionsNumber_temp);
}
...
void SYSMPU_SetRegionConfig(SYSMPU_Type *base,
    ...
    assert(regionConfig);
    assert(regionConfig->regionNum <
           (uint32_t)FSL_FEATURE_SYSMPU_DESCRIPTOR_COUNT);
    ...
    for (msPortNum = 0; msPortNum < SYSMPU_MASTER_RWATTRIBUTE_START_PORT;
         msPortNum++) {
        wordReg |= SYSMPU_REGION_RWXRIGHTS_MASTER(
            msPortNum,
            (((uint32_t)regionConfig->accessRights1[msPortNum].superAccessRights
              << 3U) |
             (uint32_t)regionConfig->accessRights1[msPortNum]
                 .userAccessRights));

#if FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER
        wordReg |= SYSMPU_REGION_RWXRIGHTS_MASTER_PE(
            msPortNum,
    ...
    for (msPortNum = SYSMPU_MASTER_RWATTRIBUTE_START_PORT;
         msPortNum < (uint32_t)FSL_FEATURE_SYSMPU_MASTER_COUNT; msPortNum++) {
        wordReg |= SYSMPU_REGION_RWRIGHTS_MASTER(
            msPortNum,
            ((uint32_t)regionConfig
                     ->accessRights2[msPortNum -
...
#endif /* FSL_FEATURE_SYSMPU_MASTER_COUNT >                                    \
          SYSMPU_MASTER_RWATTRIBUTE_START_PORT */
...
#if FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER
    wordReg |= SYSMPU_WORD_PID(regionConfig->processIdentifier) |
               SYSMPU_WORD_PIDMASK(regionConfig->processIdMask);
#endif /* FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER */
...
void SYSMPU_SetRegionRwxMasterAccessRights(
    ...
    right |= SYSMPU_REGION_RWXRIGHTS_MASTER(
        masterNum, (((uint32_t)accessRights->superAccessRights << 3U) |
                    (uint32_t)accessRights->userAccessRights));
#if FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER
    right |= SYSMPU_REGION_RWXRIGHTS_MASTER_PE(
        masterNum, accessRights->processIdentifierEnable);
#endif /* FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER */
 ...
 * - 1. param accessRights  The pointer to the SYSMPU access rights
 * configuration. See "sysmpu_rwrights_master_access_control_t".
 */
void SYSMPU_SetRegionRwMasterAccessRights(
    ...
    right |= SYSMPU_REGION_RWRIGHTS_MASTER(
        masterNum, (((uint32_t)accessRights->readEnable << 1U) |
                    (uint32_t)accessRights->writeEnable));
    /* Set low master region access rights. */
    base->RGDAAC[regionNum] = right;
}
...
bool SYSMPU_GetSlavePortErrorStatus(SYSMPU_Type *base,
    ...
    sperr = (uint8_t)(((base->CESR & SYSMPU_CESR_SPERR_MASK) >>
                       SYSMPU_CESR_SPERR_SHIFT) &
                      (0x1U << ((uint32_t)FSL_FEATURE_SYSMPU_SLAVE_COUNT -
                                (uint32_t)slaveNum - 1U)));
...
void SYSMPU_GetDetailErrorAccessInfo(SYSMPU_Type *base, sysmpu_slave_t slaveNum,
    ...
    cesReg = (base->CESR & ~SYSMPU_CESR_SPERR_MASK) |
             ((0x1UL << ((uint32_t)FSL_FEATURE_SYSMPU_SLAVE_COUNT -
                         (uint32_t)slaveNum - 1U))
              << SYSMPU_CESR_SPERR_SHIFT);
    base->CESR = cesReg;
}
