...
/*! @brief SYSMPU the bit shift for masters with privilege rights: read write
 * and execute. */
#define SYSMPU_REGION_RWXRIGHTS_MASTER_SHIFT(n) ((n) * 6U)

/*! @brief SYSMPU masters with read, write and execute rights bit mask. */
#define SYSMPU_REGION_RWXRIGHTS_MASTER_MASK(n)                                 \
...
/*! @brief SYSMPU masters with read, write and execute rights process enable bit
 * shift. */
...
/*! @brief SYSMPU masters with read, write and execute rights process enable
 * setting. */
...
#define SYSMPU_REGION_RWRIGHTS_MASTER_SHIFT(n)                                 \
    (((n) - SYSMPU_MASTER_RWATTRIBUTE_START_PORT) * 2U + 24U)

/*! @brief SYSMPU masters with normal read write rights bit mask. */
#define SYSMPU_REGION_RWRIGHTS_MASTER_MASK(n)                                  \
...
#define SYSMPU_REGION_RWRIGHTS_MASTER(n, x)                                    \
    (((uint32_t)(((uint32_t)(x)) << SYSMPU_REGION_RWRIGHTS_MASTER_SHIFT(n))) & \
...
typedef enum _sysmpu_supervisor_access_rights {
    kSYSMPU_SupervisorReadWriteExecute =
        0U, /*!< Read write and execute operations are allowed in supervisor
               mode. */
    kSYSMPU_SupervisorReadExecute =
        1U, /*!< Read and execute operations are allowed in supervisor mode. */
    kSYSMPU_SupervisorReadWrite =
        2U, /*!< Read write operations are allowed in supervisor mode. */
...
typedef enum _sysmpu_user_access_rights {
    ...
    kSYSMPU_UserWriteExecute =
        3U, /*!< Write and execute operations are allowed in user mode. */
    kSYSMPU_UserRead = 4U, /*!< Read is allowed in user mode. */
    kSYSMPU_UserReadExecute =
        5U, /*!< Read and execute operations are allowed in user mode. */
    kSYSMPU_UserReadWrite =
        6U, /*!< Read and write operations are allowed in user mode. */
    kSYSMPU_UserReadWriteExecute =
        7U /*!< Read write and execute operations are allowed in user mode. */
} sysmpu_user_access_rights_t;

/*! @brief SYSMPU hardware basic information. */
typedef struct _sysmpu_hardware_info {
    uint8_t hardwareRevisionLevel; /*!< Specifies the SYSMPU's hardware and
                                      definition reversion level. */
    uint8_t slavePortsNumbers;     /*!< Specifies the number of slave ports
                                      connected to SYSMPU. */
    sysmpu_region_total_num_t
        regionsNumbers; /*!< Indicates the number of region descriptors
                           implemented. */
...
typedef struct _sysmpu_access_err_info {
    ...
    uint8_t
        processorIdentification; /*!< Access error processor identification. */
...
typedef struct _sysmpu_rwxrights_master_access_control {
    sysmpu_supervisor_access_rights_t
        superAccessRights; /*!< Master access rights in supervisor mode. */
    sysmpu_user_access_rights_t
        userAccessRights; /*!< Master access rights in user mode. */
#if FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER
    bool
        processIdentifierEnable; /*!< Enables or disables process identifier. */
#endif                           /* FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER */
 ...
 * 0 ~ 3 with the privilege rights setting. The accessRights2[0] ~
 * accessRights2[3] are used to configure the high master 4 ~ 7 with the normal
 * read write permission. The master port assignment is the chip configuration.
 * Normally, the core is the master 0, debugger is the master 1. Note that the
 * SYSMPU assigns a priority scheme where the debugger is treated as the highest
 * priority master followed by the core and then all the remaining masters.
 * SYSMPU protection does not allow writes from the core to affect the
 * "regionNum 0" start and end address nor the permissions associated with the
 * debugger. It can only write the permission fields associated with the other
 * masters. This protection guarantees that the debugger always has access to
 * the entire address space and those rights can't be changed by the core or any
 * other bus master. Prepare the region configuration when regionNum is 0.
 */
typedef struct _sysmpu_region_config {
    uint32_t regionNum;    /*!< SYSMPU region number, range form 0 ~
                              FSL_FEATURE_SYSMPU_DESCRIPTOR_COUNT - 1. */
    uint32_t startAddress; /*!< Memory region start address. Note: bit0 ~ bit4
                              always be marked as 0 by SYSMPU. The actual start
                              address is 0-modulo-32 byte address.  */
    uint32_t endAddress;   /*!< Memory region end address. Note: bit0 ~ bit4
                            always be marked as 1 by SYSMPU. The actual   end
                            address is 31-modulo-32 byte address. */
    sysmpu_rwxrights_master_access_control_t
        accessRights1[4]; /*!< Masters with read, write and execute rights
                             setting. */
    sysmpu_rwrights_master_access_control_t
        accessRights2[4]; /*!< Masters with normal read write rights setting. */
#if FSL_FEATURE_SYSMPU_HAS_PROCESS_IDENTIFIER
 ...
 *
 * This function configures the SYSMPU module with the user-defined
 * configuration.
 *
...
static inline void SYSMPU_Enable(SYSMPU_Type *base, bool enable) {
    ...
    } else { /* Disable the SYSMPU globally. */
        base->CESR &= ~SYSMPU_CESR_VLD_MASK;
    }
}
 ...
 * @param enable   True enable the special region SYSMPU, false disable the
 * special region SYSMPU.
 */
static inline void SYSMPU_RegionEnable(SYSMPU_Type *base, uint32_t number,
                                       bool enable) {
    if (enable) {
        /* Enable the #number region SYSMPU. */
        base->WORD[number][3] |= SYSMPU_WORD_VLD_MASK;
    } else { /* Disable the #number region SYSMPU. */
        base->WORD[number][3] &= ~SYSMPU_WORD_VLD_MASK;
    }
}
 ...
 *
 * @param base           SYSMPU peripheral base address.
 * @param hardwareInform The pointer to the SYSMPU hardware information
 * structure. See "sysmpu_hardware_info_t".
 */
 ...
 *
 * Note: Due to the SYSMPU protection, the region number 0 does not allow writes
 * from core to affect the start and end address nor the permissions associated
 * with the debugger. It can only write the permission fields associated with
 * the other masters.
 *
 * @param base          SYSMPU peripheral base address.
 * @param regionConfig  The pointer to the SYSMPU user configuration structure.
 * See "sysmpu_region_config_t".
 */
 ...
 *
 * Memory region start address. Note: bit0 ~ bit4 is always marked as 0 by
 * SYSMPU. The actual start address by SYSMPU is 0-modulo-32 byte address.
 ...
 */
void SYSMPU_SetRegionAddr(SYSMPU_Type *base, uint32_t regionNum,
                          uint32_t startAddr, uint32_t endAddr);

/*!
 * @brief Sets the SYSMPU region access rights for masters with read, write, and
 * execute rights. The SYSMPU access rights depend on two board classifications
 * of bus masters. The privilege rights masters and the normal rights masters.
 ...
 * @param accessRights  The pointer to the SYSMPU access rights configuration.
 * See "sysmpu_rwxrights_master_access_control_t".
 */
void SYSMPU_SetRegionRwxMasterAccessRights(
    SYSMPU_Type *base, uint32_t regionNum, uint32_t masterNum,
    const sysmpu_rwxrights_master_access_control_t *accessRights);
#if FSL_FEATURE_SYSMPU_MASTER_COUNT > 4
/*!
 * @brief Sets the SYSMPU region access rights for masters with read and write
 * rights. The SYSMPU access rights depend on two board classifications of bus
 * masters. The privilege rights masters and the normal rights masters. The
 * normal rights masters only have the read and write access permissions. The
 * privilege rights access control see "SYSMPU_SetRegionRwxMasterAccessRights".
 *
 * @param base          SYSMPU peripheral base address.
 * @param regionNum     SYSMPU region number. The range is from 0 to
 * FSL_FEATURE_SYSMPU_DESCRIPTOR_COUNT - 1.
 * @param masterNum     SYSMPU bus master number. Should range from
 * SYSMPU_MASTER_RWATTRIBUTE_START_PORT to ~ FSL_FEATURE_SYSMPU_MASTER_COUNT
 * - 1.
 * @param accessRights  The pointer to the SYSMPU access rights configuration.
 * See "sysmpu_rwrights_master_access_control_t".
 */
void SYSMPU_SetRegionRwMasterAccessRights(
    SYSMPU_Type *base, uint32_t regionNum, uint32_t masterNum,
 ...
 * @param base       SYSMPU peripheral base address.
 * @param slaveNum   SYSMPU slave port number.
 * @param errInform  The pointer to the SYSMPU access error information. See
 * "sysmpu_access_err_info_t".
 */
void SYSMPU_GetDetailErrorAccessInfo(SYSMPU_Type *base, sysmpu_slave_t slaveNum,
