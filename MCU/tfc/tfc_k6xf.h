 ...
 * \n The pins A1/A2 for the bridge/motor A and pins B1/B2 for the bridge/motor
 * B are used on the board.						\n One
 * pin is also initialized to enable/disable both H-bridges.
 * \n
 */
 ...
 * \n When onoff is set to non-zero, function sets PWM of both motors to 0,
 * starts FTM timer and both H-bridges are enabled.	\n When onoff is set to
 * zero, function sets PWM of both motors to 0, stops FTM timer and both
 * H-bridges are disabled.		\n Calling this function with the same
 * value as the current status causes the call to be ignored.
 * \n
 *
 ...
 * \n If the value given is not within the allowed range, it is limited into
 * range <-::TFC_PWM_MINMAX, ::TFC_PWM_MINMAX>.	\n (For the safety reasons both
 * values are limited into range <-::HW_TFC_PWM_MAX, ::HW_TFC_PWM_MAX>
 * \n Selected value is used to calculate corresponding length of a PWM time
 * period for each channel.						\n PWM
 * values are then set to the both pairs of channels of the timer.
 * \n
 *
 ...
 *
 * This function initializes FTM timer and allows updating by a Software
 * Trigger.					\n The pins SERVO 0 and 1 are
 * used on the board.
 * \n
 */
 ...
 * When onoff is set to non-zero value, the function enables FTM timer.
 \n
 * When onoff is set to 0, function enables interrupt, which disables the timer
 at the end of the current PWM period,
 * to prevent a distortion of the current pulse.
 \n
 * Calling this function with the same value as the current status causes the
 call to be ignored.						\n
 //
 *
 * @param onoff	The non-zero value - ON, the 0 value - OFF
 ...
 *
 * This function takes two arguments, first is used to determine which servo is
 * being set.                           .	\n Second parameter is value to
 * set the position of the servo.
 * \n If the value given is not within the allowed range, it is set into range
 * <-::TFC_SERVO_MAX_LR+::TFC_SERVO_DEFAULT_CENTER,
 * ::TFC_SERVO_MAX_LR+::TFC_SERVO_DEFAULT_CENTER>.
 * \n Selected value is used to calculate corresponding length of a PWM time
 * period.
 ...
 *
 * This function initializes ADC, which reads values of potentiometers, H-bridge
 * feedbacks and battery voltage.	\n
 */
 ...
 *
 * This function enables or disables interrupt, which starts the ADC and
 * converts all values.			\n Calling this function with the same
 * value as the current status causes the call to be ignored. \n
 *
 * @param onoff The non-zero value - ON, the 0 value - OFF
 ...
 *
 * This function enables or disables interrupt, which starts the ADC conversion
 * of pixels [::TFC_CAMERA_LINE_LENGTH] for both cameras.		\n
 * Calling this function with the same value as the current status causes the
 * call to be ignored.
 * \n
 *
...
/**
 * @brief Function to copy camera image of selected camera from internal buffer
 * into array.
 *
