...
uint32_t TFC::getDIPSwitch() { return HW_TFC_INPIN_DIPSwitch(); }
...
float TFC::ReadBatteryVoltage_f() {
    int val = ReadADC(anBAT);
    return ((float)val) / TFC_ADC_MAXVAL * 3.3 *
           5.7; // * ((47000.0+10000.0)/10000.0);
}
...
void TFC::InitRC() { HW_TFC_RC_Init(); }

// enable/disable pulses detection on SPEED0 and SPEED1 pins
void TFC::RCOnOff(uint32_t onoff) { HW_TFC_RC_OnOff(onoff); }
