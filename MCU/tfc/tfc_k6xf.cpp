...
#define HW_TFC_MOTOR_PIN_FAULT                                                 \
    3 // Fault status from H-Bridges (2x Open Collector)
...
#define HW_TFC_MOTOR_PIN_FAULT                                                 \
    0 // Fault status from H-Bridges (2x Open Collector)
...
#define HW_TFC_MOTOR_PIN_FAULT                                                 \
    8 // Fault status from H-Bridges (2x Open Collector)
...
void HW_TFC_MOTOR_Init() {
    ...
    PORT_SetPinMux(HW_TFC_MOTOR_PORT_SIG, HW_TFC_MOTOR_PIN_ENABLE,
                   kPORT_MuxAsGpio);
    PORT_SetPinMux(HW_TFC_MOTOR_PORT_SIG, HW_TFC_MOTOR_PIN_FAULT,
                   kPORT_MuxAsGpio);
    ...
    FTM_SetupPwm(HW_TFC_MOTOR_FTM, &ftmpar, 1, kFTM_EdgeAlignedPwm,
                 HW_TFC_MOTOR_FREQ, CLOCK_GetBusClkFreq());
...
void HW_TFC_MOTOR_OnOff(int onoff) {
    ...
    if (onoff) {
        ...
        GPIO_SetPinsOutput(HW_TFC_MOTOR_GPIO_SIG, 1 << HW_TFC_MOTOR_PIN_ENABLE);
    } else {
        HW_TFC_MOTOR_SetPWM(0, 0);
        GPIO_ClearPinsOutput(HW_TFC_MOTOR_GPIO_SIG,
                             1 << HW_TFC_MOTOR_PIN_ENABLE);
...
void HW_TFC_SERVO_Init() {
    ...
    FTM_SetupPwm(HW_TFC_SERVO_FTM, &ftmpar, 1, kFTM_EdgeAlignedPwm,
                 HW_TFC_SERVO_FREQ, CLOCK_GetBusClkFreq());
...
void HW_TFC_SERVO_OnOff(int onoff) {
    if (!onoff == !_s_hw_tfc_servo_is_on)
        return;
...
void HW_TFC_SERVO_Set(int servo, int pos_us) {
    ...
    servo = servo != 0;
    HW_TFC_SERVO_FTM->CONTROLS[servo].CnV =
        HW_TFC_SERVO_FTM->MOD * pos_us / (1000000 /*us*/ / HW_TFC_SERVO_FREQ);
...
#define HW_TFC_ANDATA_CHN_IR_1 21                // AD channel of Battery
#define HW_TFC_ANDATA_CHN_IR_2 22                // AD channel of Battery
#define HW_TFC_ANDATA_CHN_IR_3 23                // AD channel of Battery
...
static volatile hw_tfc_anadata_chnl_t _s_andata_chnls[anLast] = {
    ...
    {HW_TFC_ANDATA_CHN_IR_1, 0}, // anIR_1
    {HW_TFC_ANDATA_CHN_IR_2, 0}, // anIR_2
    {HW_TFC_ANDATA_CHN_IR_3, 0}  // anIR_3
};
...
void HW_TFC_ANDATA_IRQHandler() {
    _s_andata_chnls[_s_andata_chnl_idx].value =
        ADC16_GetChannelConversionValue(HW_TFC_ANDATA_ADC, 0);
    ...
    if (_s_andata_chnl_idx == anLast) {
        ...
        _s_andata_chnl_conf.channelNumber =
            _s_andata_chnls[_s_andata_chnl_idx].channel;
...
void HW_TFC_ANDATA_Init() {
    ...
    PORT_SetPinMux(HW_TFC_ANDATA_PORT, HW_TFC_ANDATA_PIN_POT1,
                   kPORT_PinDisabledOrAnalog);
    PORT_SetPinMux(HW_TFC_ANDATA_PORT, HW_TFC_ANDATA_PIN_POT2,
                   kPORT_PinDisabledOrAnalog);
#endif
 ...
 */
static volatile uint16_t _sa_camera_buf[_HW_NUM_OF_CAMERAS][2]
                                       [TFC_CAMERA_LINE_LENGTH];
...
static uint32_t _s_camera_channels[2] = {HW_TFC_CAMERA0_CHNL,
                                         HW_TFC_CAMERA1_CHNL};
...
static inline void HW_TFC_CAMERA_SI_Set(int level) {
    if (level)
        GPIO_SetPinsOutput(HW_TFC_CAMERA_GPIO_SIG, 1 << HW_TFC_CAMERA_PIN_SI);
    else
        GPIO_ClearPinsOutput(HW_TFC_CAMERA_GPIO_SIG, 1 << HW_TFC_CAMERA_PIN_SI);
}
...
static inline void HW_TFC_CAMERA_CLK_Set(int level) {
    if (level)
        GPIO_SetPinsOutput(HW_TFC_CAMERA_GPIO_SIG, 1 << HW_TFC_CAMERA_PIN_CLK);
    else
        GPIO_ClearPinsOutput(HW_TFC_CAMERA_GPIO_SIG,
                             1 << HW_TFC_CAMERA_PIN_CLK);
}
...
void HW_TFC_CAMERA_IRQHandler() {
    ...
    if (_s_camera_line_idx < TFC_CAMERA_LINE_LENGTH) {
        _sp_camera_work_buf[_s_camera_idx++][_s_camera_line_idx] =
        ...
        adc16_channel_config_t chc = {_s_camera_channels[_s_camera_idx], true,
                                      false};
...
void HW_TFC_CAMERA_Init() {
    ...
    PORT_SetPinMux(HW_TFC_CAMERA_PORT_SIG, HW_TFC_CAMERA_PIN_SI,
                   kPORT_MuxAsGpio);
    PORT_SetPinMux(HW_TFC_CAMERA_PORT_SIG, HW_TFC_CAMERA_PIN_CLK,
                   kPORT_MuxAsGpio);
...
uint32_t HW_TFC_CAMERA_isReady() { return _s_camera_image_ready; }
...
#define HW_TFC_TICKER_CAMERA_PERIOD 6  // Period of camera data capturing
// #define HW_TFC_TICKER_CAMERA_PERIOD	10
// // Period of camera data capturing

/**
 * Data for timers and periodic trigger of AD conversion
 */
volatile uint32_t HW_TFC_Ticker_Up[HW_TFC_NUM_TICKERS],
    HW_TFC_Ticker_Down[HW_TFC_NUM_TICKERS];
...
uint32_t getMs() { return _HW_TFC_Ticker_ms; }
...
void HW_TFC_TICKER_IRQHandler() {
    ...
    for (int i = 0; i < HW_TFC_NUM_TICKERS; i++) {
        if (HW_TFC_Ticker_Down[i])
            HW_TFC_Ticker_Down[i]--;
        HW_TFC_Ticker_Up[i]++;
    }
...
void HW_TFC_IOPIN_Init() {
    ...
    gpio_pin_config_t pin_input = {kGPIO_DigitalInput, 0};
...
uint32_t HW_TFC_INPIN_DIPSwitch() {
    ...
    b = _INPIN_MASK(b, HW_TFC_SWPIN_SW1, 0) |
        _INPIN_MASK(b, HW_TFC_SWPIN_SW2, 1) |
        _INPIN_MASK(b, HW_TFC_SWPIN_SW3, 2) |
        _INPIN_MASK(b, HW_TFC_SWPIN_SW4, 3);
...
uint32_t HW_TFC_INPIN_ABSwitch() {
    ...
    b = _INPIN_MASK(b, HW_TFC_BUTPIN_BUTA, 0) |
        _INPIN_MASK(b, HW_TFC_BUTPIN_BUTB, 1);

    return b & 0x3;
}

void HW_TFC_OUTPIN_LED(uint32_t led, uint32_t level) {
    static uint32_t leds_pin[4] = {HW_TFC_LEDPIN_LED1, HW_TFC_LEDPIN_LED2,
                                   HW_TFC_LEDPIN_LED3, HW_TFC_LEDPIN_LED4};
    if (led > 3)
        return;
    if (level) {
        GPIO_SetPinsOutput(HW_TFC_LEDPIN_GPIO, 1 << leds_pin[led]);
    } else {
        GPIO_ClearPinsOutput(HW_TFC_LEDPIN_GPIO, 1 << leds_pin[led]);
    }
}
...
void HW_TFC_OUTPIN_LEDs(uint32_t leds) {
    ...
    uint32_t set = _OUTPIN_MASK(leds, HW_TFC_LEDPIN_LED1, 0) |
                   _OUTPIN_MASK(leds, HW_TFC_LEDPIN_LED2, 1) |
                   _OUTPIN_MASK(leds, HW_TFC_LEDPIN_LED3, 2) |
                   _OUTPIN_MASK(leds, HW_TFC_LEDPIN_LED4, 3);
    GPIO_SetPinsOutput(HW_TFC_LEDPIN_GPIO, set);
    GPIO_ClearPinsOutput(HW_TFC_LEDPIN_GPIO, set ^ mask);
}
...
void HW_TFC_SPEED0_IRQHandler() { _HW_TFC_SPEED01_IRQHandler(_hw_tfc_speed_s); }
...
void _HW_TFC_SPEED01_IRQHandler(volatile hw_tfc_speed_pin_irq_data_t *data) {
    ...
    {
        if (_HW_TFC_Ticker_ms - data->up_ms > 2)
            return; // ignore long time
...
void HW_TFC_RC_OnOff(uint32_t onoff) {
    if (onoff) {
        ...
        PORT_SetPinInterruptConfig(HW_TFC_SPEED0_PORT, HW_TFC_SPEED0_PIN,
                                   kPORT_InterruptEitherEdge);
        PORT_SetPinInterruptConfig(HW_TFC_SPEED1_PORT, HW_TFC_SPEED1_PIN,
                                   kPORT_InterruptEitherEdge);
...
void HW_TFC_SPEED_IRQHandler() {
    ...
    if (status & HW_TFC_SPEED_CHNL_1) {
        ...
        uint32_t down_time =
            HW_TFC_SPEED_FTM->CONTROLS[HW_TFC_SPEED_CHNL_1].CnV;
        _hw_tfc_speed_us[0] =
            ((up_time < down_time) ? (down_time - up_time)
                                   : (0xFFFF - up_time + down_time)) *
            1000 /*ms to us*/ / _HW_TFC_RC_1ms_ticks;
    ...
    if (status & HW_TFC_SPEED_CHNL_3) {
        ...
        uint32_t down_time =
            HW_TFC_SPEED_FTM->CONTROLS[HW_TFC_SPEED_CHNL_3].CnV;
        _hw_tfc_speed_us[1] =
            ((up_time < down_time) ? (down_time - up_time)
                                   : (0xFFFF - up_time + down_time)) *
            1000 /*ms to us*/ / _HW_TFC_RC_1ms_ticks;
...
void HW_TFC_RC_Init() {
    ...
    FTM_SetupDualEdgeCapture(HW_TFC_SPEED_FTM, HW_TFC_SPEED_CHNL_D0,
                             &dual_cap_par, 0);
    FTM_SetupDualEdgeCapture(HW_TFC_SPEED_FTM, HW_TFC_SPEED_CHNL_D1,
                             &dual_cap_par, 0);
}
...
void HW_TFC_RC_OnOff(uint32_t onoff) {
    ...
    if (onoff) {
        ...
        FTM_EnableInterrupts(HW_TFC_SPEED_FTM, HW_TFC_SPEED_CHNL_1_INTEN |
                                                   HW_TFC_SPEED_CHNL_3_INTEN);
        FTM_StartTimer(HW_TFC_SPEED_FTM, kFTM_SystemClock);
    } else {
        FTM_StopTimer(HW_TFC_SPEED_FTM);
        FTM_DisableInterrupts(HW_TFC_SPEED_FTM, HW_TFC_SPEED_CHNL_1_INTEN |
                                                    HW_TFC_SPEED_CHNL_3_INTEN);
