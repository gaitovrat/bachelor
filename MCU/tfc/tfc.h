...
When using variation of this packet to send/receive data, the data should be put
in a place of ::data. \n There can be some of the data structure (::tfc_data_s,
...
struct tfc_data_s {
    ...
    uint16_t
        adc[anLast]; ///< All analog data specified in ::tfc_andata_chnl_enum.
...
class TFC {
  public:
    ...
    /**
     * Initialize all variables to it's default values and initialize all HW
     * parts.
     */
    void InitAll();
    ...
    /**
     * Set multiple LEDs.
     * @param leds 4 LEDs in the form of a low nibble (the lowest 4 bits of the
     * variable), where each bit value {0,1} turns on/off its corresponding
     * LEDs.
     */
    ...
    /** @name Buttons and DIP switch
     These methods are used to obtain values from the DIP switches or push
     buttons.\n
     */
    ///@{
     ...
     * @param channel Corresponds to a button, 0 - the first button, any other
     ...
     * @param andata_chnl Type of a analog value to get. It can be any value
     * from ::tfc_andata_chnl_enum.
    ...
    /** @name Potentiometers
     ...
     +-------------------------|-------------------------+ the range is
     translated to this:                                   0 TFC_ADC_MAXVAL/2
     TFC_ADC_MAXVAL |                         |                         | The
     ReadPot_i() method translates the ADC values
     +-------------------------|-------------------------+ to this range:
     -TFC_ANDATA_MINMAX                0               TFC_ANDATA_MINMAX | | |
     ...
     * @param Channel 0 - the first potentiometer, any other value - the second
     potentiometer.
     * @return Current value in range <-::TFC_ANDATA_MINMAX ,
     ::TFC_ANDATA_MINMAX >.
     */
    ...
    /** @name Feedback
     ...
     +---------------------------------------------------+ from the ADC, the
     range looks like this:                           0 TFC_ADC_MAXVAL | | The
     ReadFB_i() method translates the ADC values
     +---------------------------------------------------+ to this range: 0
     TFC_ANDATA_MINMAX |                                                   | The
     ReadFB_f() method further translates
     +---------------------------------------------------+ the value to the
     current electrical current in Amps: 0 5.625 [Amp]
     @endcode
     */
    ///@{
     ...
     * @param Channel 0 - the first FB channel, any other value - the second FB
     * channel.
    ...
    /** @name Battery
     ...
     +---------------------------------------------------+ from the ADC, the
     range looks like this:                           0 TFC_ADC_MAXVAL | | The
     ReadBatteryVoltage_i() method translates
     +---------------------------------------------------+ the ADC values to
     this range:                                      0 TFC_ANDATA_MINMAX | |
     ...
     * @param channel 0 - the first camera, any other value - the optional
     * second camera.
     ...
     * @param channel 0 - the first camera, any other value - the optional
     * second camera.
     * @param img Pointer to an uint16_t array representing the line image. Data
     * in range <0, TFC_ADC_MAXVAL>
    ...
    /** @name Servos
     ...
     +-------------------------|-------------------------+ so their value range
     looks by default like this:      (TFC_SERVO_DEFAULT_CENTER
     (TFC_SERVO_DEFAULT_CENTER)  (TFC_SERVO_DEFAULT_CENTER (or it can be
     calibrated by setServoCalibration())    -TFC_SERVO_DEFAULT_MAX_LR)
     +TFC_SERVO_DEFAULT_MAX_LR) |                         | | The methods
     getServo_i() and setServo_i()
     +-------------------------|-------------------------+ use value in this
     range:                                    -TFC_SERVO_MINMAX 0
     TFC_SERVO_MINMAX |                         |                         | The
     setServo_f() method further translates
     +-------------------------|-------------------------+ the values to this
     range:                                        -1.0
     0                        1.0
     @endcode
    ...
    /**
     * Servo calibration.
     * @param channel Channel 0 - the first servo, any other value - the second
     * servo.
     * @param center Pulse width in microseconds corresponding to the center
     * position of the servo.
     * @param max_lr Offset from center to either side corresponding to the
     * maximal rotation, in the form of a pulse width in microseconds.
     */

    void setServoCalibration(uint32_t channel, uint32_t center,
                             uint32_t max_lr);
    ...
    /** @name Motors
     These methods are used to control the power and the direction of rotation
     of motors.\n
     ...
     +-------------------------|-------------------------+ (negative values
     correspond to reverse direction):               -100% 0% 100% | | | The
     methods getMotorPWM_i() and setMotorPWM_i()
     +-------------------------|-------------------------+ use value in this
     range:                                    -TFC_PWM_MINMAX 0 TFC_PWM_MINMAX
                                                                        | | |
     ...
     * This limits the power of motors to the given value, so when the maximal
     * value is set by this method for example to 300 and then the method
     * TFC::setMotorPWM_i() will be called with values greater than this one,
     * the motors power will be reduced to just 300.
     ...
     * If the values are greater than the maximum set by the method
     * TFC::setPWMMax(), the motors will only run at that set maximum.
     ...
     * @param channel_ab Channel 0 - the first motor, any other value - the
     * second motor.
    ...
    /** @name Remote Control
     These methods are used to control and obtain data from the remote control
     receiver.
    */
    ///@{
